(function () {
    ...
    function initWidget() {
        let socket;
        let phone = sessionStorage.getItem('chat_phone');
        ...
        function initSocket() {
            socket = io(APP_URL, {
                query: { token, userPhone: phone, type: 'user' }
            });
        ...
        function appendMessagesToChat(messageList) {
            messageList.map((msg, i) => {
                const isMine = msg.senderType == 'user' ? true : false;
                const msgDiv = document.createElement('div');
                const messageText = typeof msg === 'string' ? msg : msg.content;
                msgDiv.textContent = messageText;
